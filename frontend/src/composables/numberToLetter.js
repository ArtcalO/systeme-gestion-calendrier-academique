...
const NumberToLetter = (nombre)=>{
	...
	nb = Math.ceil(parseFloat(nombre.toString().replace(/ /gi, "")));
	...
	switch (n) {
		...
		case 2: if (nb > 19) {
			...
			if (nb < 71 || (nb > 79 && nb < 91)) {
				...
				if (reste == 1) numberToLetter = Dizaine(quotient * 10) + "-et-" + this.Unite(reste);
				if (reste > 1) numberToLetter = Dizaine(quotient * 10) + "-" + this.Unite(reste);
