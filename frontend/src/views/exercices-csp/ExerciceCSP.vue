<script >
import ExercicesResponses from "../../components/reusables/ExercicesCSPResponse.vue"
	...
	methods:{
		...
		validateResponses(){
			if(this.responses.length==0){
				...
				axios
					.post(`reponseEleveCSP/`, {"reponses":this.responses,"eleve":this.getEleveId('eleve')}, this.headers)
					.then((res) => {
		...
		fetchExcercices(){
			this.isLoading=true
			axios.get('exerciceCSP/?chapitre='+this.$route.params.id)
			.then((res)=>{
				this.isLoading=false
				this.exercices=res.data.results
			})
