...
export default {
	data(){
		return {
			...
			isLoading:false,keyword:"",Plus,Search,chapitre:{},responses:[]
		}
	},
	...
	methods:{
		fetchChapitre(){
			this.isLoading=true
			axios.get('chapitres/'+this.$route.params.id+'/')
			.then((res)=>{
			...
			.catch((err)=>{
				this.isLoading=false
				this.errorOrRefresh(err, this.fetchChapitre)
			})
		},
		fetchExercices() {
			this.fetching=true
			axios
				.get(`${this.url}/exercicesCSP/?chapitre=${this.$route.params.id}`, this.headers)
				.then((res) => {
		...
		validateResponses(){
			if(this.responses.length==0){
				...
				axios
					.post(`reponsesElevesCSP/`, {"reponses":this.responses,"eleve":this.getEleveId('eleve')}, this.headers)
					.then((res) => {
		...
		fetchExcercices(){
			this.isLoading=true
			axios.get('exercicesCSP/?chapitre='+this.$route.params.id)
			.then((res)=>{
				this.isLoading=false
				this.exercices=res.data.results
			})
