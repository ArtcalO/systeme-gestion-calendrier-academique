<template>
  <div class="subjects-page">
    <div class="page-header">
      <div>
        <h2>Matières / Domaines</h2>
        <p class="subtitle">Gestion des matières et domaines d'enseignement</p>
      </div>
      <el-button type="primary" :icon="Plus" @click="openModal()">Nouvelle matière</el-button>
    </div>
    ...
    <el-card shadow="never" class="mt-3">
      <el-table :data="subjects" v-loading="loading" style="width:100%" stripe>
        <el-table-column label="Code" width="110">
          <template #default="{ row }">
            <el-tag type="success" size="small">{{ row.code }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="name" label="Matière/Domaine" min-width="220" />
    ...
    <el-dialog v-model="showModal" :title="editItem ? 'Modifier matière' : 'Nouvelle matière'" width="460px">
      <el-form :model="form" :rules="rules" ref="formRef" label-position="top">
        <el-row :gutter="16">
          <el-col :span="16">
            <el-form-item label="Nom" prop="name">
              <el-input v-model="form.name" placeholder="ex: Mathématiques" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="Code" prop="code">
              <el-input v-model="form.code" placeholder="ex: MATH" />
            </el-form-item>
          </el-col>
        </el-row>
...
async function load() {
  ...
  try {
    ...
    const res = await axios.get('/academic/subjects/', { params })
    const d = res.data
    subjects.value = d.results || d
    total.value = d.count || subjects.value.length
...
async function save() {
  ...
  saving.value = true
  try {
    if (editItem.value) await axios.patch(`/academic/subjects/${editItem.value.id}/`, form)
    else await axios.post('/academic/subjects/', form)
...
async function handleDelete(row) {
  await ElMessageBox.confirm('Supprimer cette matière ?', 'Confirmation', { type: 'warning' })
  await axios.delete(`/academic/subjects/${row.id}/`)
...
<style scoped>
.subjects-page { max-width: 1200px; }
